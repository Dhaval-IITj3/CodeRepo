...
def plot_fits(num_points, degrees, x_train, y_train, x_test, y_test, x_new, y_new_pred_list, mse_train_list, mse_test_list):
    plt.figure(figsize=(18, 24))
    plt.suptitle(f'Polynomial Regression for {num_points} Data Points')

    for i, degree in enumerate(degrees):
        plt.subplot(3, 3, i + 1)

        plt.scatter(x_train, y_train, color='blue', label='Training data', marker='.')
        plt.scatter(x_test, y_test, color='orange', label='Testing data', marker='.')
    ...
    plt.subplot(3, 3, 9)
    plt.plot(degrees, mse_train_list, marker='.', label='Training MSE')
    plt.plot(degrees, mse_test_list, marker='.', label='Testing MSE')
    plt.xlabel('Polynomial Degree')
    plt.ylabel('Mean Squared Error')
    plt.title(f'MSE Comparison')
    plt.legend()

    plt.subplots_adjust(bottom=0.06, top=0.93, hspace=0.36, wspace=0.3)
    plt.show()
...
def analyze_polynomial_regression(num_points_list, degrees):
    ...
    for num_points in num_points_list:
        # Generate data
        x, y_true, y_noisy = generate_data(num_points, noise_std=0.03)
        ...
        for degree in degrees:
            model, poly_features, mse_train, mse_test = fit_polynomial_regression(x_train, y_train, x_test, y_test, degree)
        ...
        plot_fits(num_points, degrees, x_train, y_train, x_test, y_test, x_new, y_new_pred_list, mse_train_list, mse_test_list)
    ...
    plt.plot(degrees, mse_train_50, marker='.', label='Training MSE (50 points)')
    plt.plot(degrees, mse_test_50, marker='.', label='Testing MSE (50 points)')
    plt.plot(degrees, mse_train_1000, marker='.', linestyle='--', label='Training MSE (1000 points)')
    plt.plot(degrees, mse_test_1000, marker='.', linestyle='--', label='Testing MSE (1000 points)')
